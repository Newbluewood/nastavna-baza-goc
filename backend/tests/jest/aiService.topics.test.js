// Testovi za sve glavne teme i nove JSON-ove
const aiService = require('../../services/aiService');

describe('aiService.processAssistantMessageV2 - svi topici', () => {
  const topicTests = [
  ...
  topicTests.forEach(({ question, topic, expectField }) => {
    it(`vraa podatke za temu: ${topic}`, async () => {
      ...
      if (topic === 'attraction') {
        // Debug output for attraction topic
        // eslint-disable-next-line no-console
        console.log('DEBUG attraction facts:', JSON.stringify(res.facts, null, 2));
        console.log('DEBUG filtered facts:', JSON.stringify(facts, null, 2));
      }
      expect(facts.length).toBeGreaterThan(0);
      facts.forEach(item => {
        expect(item[expectField]).toBeDefined();
      });
    });
  });
});
