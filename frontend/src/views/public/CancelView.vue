...
import { fmt, daysUntil } from '../../utils/dateFormat'
...
onMounted(async () => {
  try {
    reservation.value = await api.getCancelInfo(token)
  } catch (e) {
    error.value = e.data?.error || 'Rezervacija nije pronađena.'
  } finally {
...
const cancelReservation = async () => {
  ...
  try {
    ...
    error.value = e.data?.error || 'Greška pri otkazivanju. Pokušajte ponovo.'
  } finally {
