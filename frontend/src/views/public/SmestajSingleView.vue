      ...
      <div v-if="building.rooms && building.rooms.length > 0" class="facilities-grid">
        <div v-for="room in building.rooms" :key="room.id" class="facility-card">
          ...
          <div class="facility-info">
            ...
            <!-- Price Info -->
            <div class="price-info-block" v-if="room.price_base">
              <div class="price-main">
                <span class="price-label">{{ langStore.currentLang === 'sr' ? 'Цене од' : 'Prices from' }}:</span>
                <span class="price-amount">{{ room.price_base }} RSD</span>
                <span class="price-unit">{{ langStore.currentLang === 'sr' ? '/ ноћ по особи' : '/ night per person' }}</span>
              </div>
              <div class="price-details" v-if="room.price_half_board || room.price_full_board">
                <span v-if="room.price_half_board">☕ {{ langStore.currentLang === 'sr' ? 'Полупансион' : 'Half Board' }}: <strong>{{ room.price_half_board }} RSD</strong></span>
                <span v-if="room.price_full_board">🍽️ {{ langStore.currentLang === 'sr' ? 'Пун пансион' : 'Full Board' }}: <strong>{{ room.price_full_board }} RSD</strong></span>
              </div>
              <p v-if="room.meal_info" class="meal-disclaimer">{{ room.meal_info }}</p>
            </div>
