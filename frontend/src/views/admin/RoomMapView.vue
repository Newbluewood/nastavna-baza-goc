...
const capShort = { single: 'јед', double: 'дво', triple: 'тро', multi: 'више' }

function roomCapLabel(room) {
  const max = room.capacity_max || 1
  return `${max}.${capShort[room.capacity_type] || 'више'}`
}
...
async function fetchRoomMap() {
  ...
  tooltip.value = null
  try {
    const data = await api.getRoomMap(selectedDate.value)
